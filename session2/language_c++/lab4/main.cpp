...
private:
    void q_string(char menuq)
    {
        ...
        while (true)
        {
            if (keys->data == nullptr)
                return;
            if (menuq == '1' && keys->data->title == etalon)
                keys->print_data();
            if (menuq == '2' && keys->data->color == etalon)
                keys->print_data();
            if (keys->next == nullptr)
                return;
            keys = keys->next;
    ...
    void q_integer(char menuq)
    {
        ...
        while (true)
        {
            if (keys->data == nullptr)
                return;
            if (menuq == '3')
                data_inf = keys->data->price;
            if (menuq == '4')
                data_inf = keys->data->materials.natural;
            if (menuq == '5')
                data_inf = keys->data->materials.artificial;
            if (filter(etalon, data_inf, integer))
                keys->print_data();
            if (keys->next == nullptr)
                return;
            keys = keys->next;
